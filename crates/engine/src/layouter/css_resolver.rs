...
use crate::css::parser::{AtQuery, ComplexSelector, CssNode, CssNodeType};
use crate::css::values::{CssIdent, CssValue, Unit};
...
impl<'a> RuleSet<'a> {
    ...
    pub fn query_candidates(
        &self,
        element: &crate::css::matcher::ElementInfo,
...
pub fn resolve_inline_style(style_attr: &str) -> Vec<(String, CssValue, bool)> {
    let mut parser = crate::css::parser::Parser::new(style_attr);
...
pub fn resolve_inline_value(value: &str) -> Option<CssValue> {
    let mut tokenizer = crate::css::tokenizer::Tokenizer::new(value);
    let mut tokens = Vec::new();

    loop {
        let token = tokenizer.next_token();
        if token == crate::css::tokenizer::Token::EOF {
            break;
        }
    ...
    let Ok(value) = crate::css::parser::Parser::parse_tokens_to_css_value(tokens) else {
        return None;
    };
...
impl CssResolver {
    ...
    pub fn resolve_inline_style(style: &str) -> ResolvedStyles {
        ...
        let Ok(stylesheet) = crate::css::parser::Parser::new(&source).parse() else {
            return Vec::new();
        };
...
mod tests {
    ...
    fn test_rule_set_partitioning_and_querying() {
        ...
        let el1 = crate::css::matcher::ElementInfo {
        ...
        let el2 = crate::css::matcher::ElementInfo {
