...
use engine::image_decoder::ImageDecoder;
use engine::layouter::types::{ColorScheme, TextFlowStyle};
use engine::{
    css::{
        self,
    ...
    pub fn needs_redraw(&self) -> bool {
        self.needs_redraw
            || self
                .layout_and_info
                .as_ref()
                .is_some_and(|(_, info)| engine::input::any_custom_node_needs_repaint(info))
    }
...
mod tests {
    use super::*;
    use engine::layouter::types::{ContainerRole, ContainerStyle};
    ...
    fn applied_layout_results_are_positioned_before_any_draw() {
        ...
        let path = engine::input::hit_test(layout, info, 50.0, 25.0);
        assert!(
            engine::input::hit_dom_id(&path).is_some(),
