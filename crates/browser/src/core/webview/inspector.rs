...
use engine::css::values::CssValue;
use engine::html::HtmlNodeType;
use engine::layouter::css_resolver::{MediaEnvironment, RuleSet};
use engine::layouter::dom_snapshot::NodeId;
use engine::layouter::style_inspect::{MatchedRule, collect_matched_rules};
use engine::layouter::types::InfoNode;
use engine::tree::{NodeRef, TreeNode};
...
fn serialize_origin(origin: engine::layouter::css_resolver::StyleOrigin) -> &'static str {
    match origin {
        engine::layouter::css_resolver::StyleOrigin::UserAgent => "user-agent",
        engine::layouter::css_resolver::StyleOrigin::Author => "author",
    }
}
...
fn layout_target(webview: &mut WebView, dom_id: u64) -> Result<(NodeId, LayoutTarget<'_>), String> {
    ...
    if matches!(layout.layout_box, ui_layout::LayoutBox::None) {
        ...
        if engine::layouter::constrain_auto_grid_track_items(layout) {
            ui_layout::LayoutEngine::layout(layout, webview.viewport.0, webview.viewport.1);
        }
    }
...
mod tests {
    use super::*;
    use engine::html::parser::Parser as HtmlParser;
