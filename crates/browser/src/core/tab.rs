...
impl Tab {
    ...
    pub fn handle_mouse_input(&mut self, px: f32, py: f32, state: ElementState) -> (bool, bool) {
        ...
        let dom_id = engine::input::hit_dom_id(&path);

        let mut repaint = false;

        match state {
            ElementState::Pressed => {
                engine::input::dismiss_open_popups(info, &path);
                ...
                let input_focused = engine::input::focus_text_input(info, input_target.as_ref());

                engine::input::dispatch_pointer(&path, PointerEvent::Down { x: px, y: py });
            ...
            ElementState::Released => {
                engine::input::dispatch_pointer(&path, PointerEvent::Up { x: px, y: py });
    ...
    pub fn handle_pointer_move(&mut self, px: f32, py: f32) -> (bool, bool) {
        ...
        let input_focused =
            engine::input::dispatch_pointer(&move_path, PointerEvent::Move { x: px, y: py });

        let hover_changed = {
            let previous = self.hovered.as_ref();
            engine::input::update_hover(&move_path, previous)
        };

        let mut repaint = false;
        if hover_changed {
            repaint = true;
            self.hovered = engine::input::hit_custom_node(&move_path).cloned();
        }
    ...
    pub fn dispatch_text_input(&self, event: InputTextEvent) -> bool {
        ...
        engine::input::dispatch_text_input(info, event)
    }
    ...
    pub fn defocus_text_input(&self) -> bool {
        ...
        engine::input::focus_text_input(info, None)
    }
    ...
    pub fn link_at(&self, px: f32, py: f32) -> Option<String> {
        ...
        let path = engine::input::hit_test(layout, info, px, py);
    ...
    pub fn scroll_at(&mut self, px: f32, py: f32, dx: f32, dy: f32, viewport: (f32, f32)) {
        ...
        engine::input::scroll_at(layout, info, viewport, px, py, dx, dy);
    }
    ...
    pub fn is_text_input_composing(&self) -> bool {
        ...
        engine::input::focused_text_input_is_composing(info)
    }
