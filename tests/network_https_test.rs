...
fn test_http_and_https_comparison() -> Result<(), Box<dyn Error>> {
    ...
    rt.block_on(async {
        ...
        assert!(
            https_response.status.is_success(),
            "HTTPS request failed with status: {}",
            https_response.status
        );

        assert!(
            !http_response.body.is_empty(),
            "HTTP response body should not be empty"
        );
        assert!(
            !https_response.body.is_empty(),
            "HTTPS response body should not be empty"
        );
