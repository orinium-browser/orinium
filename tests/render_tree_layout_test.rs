...
use orinium_browser::engine::renderer::{RenderTree, NodeKind};
...
impl TextMeasurer for MockMeasurer {
    fn measure(&self, _req: &TextMeasurementRequest) -> Result<TextMeasurement, TextMeasureError> {
        Ok(TextMeasurement { width: 50.0, height: 12.0, baseline: 9.0, glyphs: None })
    }
}
...
fn render_tree_uses_measurer() {
    ...
    assert!(html_weak.upgrade().is_some(), "html_text_node weak upgrade failed");
    assert!(root_weak.upgrade().is_some(), "root_html_node weak upgrade failed");
    ...
    if let NodeKind::Scrollable { tree: inner_tree, .. } = &root_node.value.kind {
        ...
        assert!((rn.height - 12.0).abs() < 1e-6, "height not set by measurer");
    } else {
